  ...
  useEffect(() => {
    ...
    try {
      ...
      Promise.all(
        ...
        // Correct CAD orientation on inner assembly before bounding box
        if (rotationX !== 0) assembly.rotation.x = rotationX
        // Center + scale the spinner (outer group) so world origin is the spin center
        const box = new Box3().setFromObject(spinner)
        const center = box.getCenter(new Vector3())
        const size = box.getSize(new Vector3())
        spinner.position.copy(center).negate()
        spinner.scale.setScalar(3.8 / Math.max(size.x, size.y, size.z))
        scene.add(spinner)
        setLoading(false)
      })
      ...
      function animate() {
        ...
        if (mini || isTouch) {
          // Rotate spinner (outer) around world Y — keeps orientation fix intact
          rotY += 0.008
